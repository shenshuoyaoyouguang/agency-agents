...
#   trae         — Single project_rules.md for Trae
...
# Aider, Windsurf and Trae are single-file formats — accumulate into temp files
# then write at the end.
AIDER_TMP="$(mktemp)"
WINDSURF_TMP="$(mktemp)"
TRAE_TMP="$(mktemp)"
trap 'rm -f "$AIDER_TMP" "$WINDSURF_TMP" "$TRAE_TMP"' EXIT
...
cat > "$TRAE_TMP" <<'HEREDOC'
# The Agency — AI Agent Rules for Trae
#
# Full roster of specialized AI agents from The Agency.
# To activate an agent, reference it by name in your Trae conversation.
#
# Generated by scripts/convert.sh — do not edit manually.
...
main() {
  ...
  if $use_parallel && [[ "$tool" == "all" ]]; then
    ...
    local parallel_tools=(antigravity gemini-cli opencode cursor openclaw qwen mcp-server)
    ...
    local idx=9
    for t in aider windsurf trae; do
...
HEREDOC
  ...
  if [[ "$tool" == "all" || "$tool" == "trae" ]]; then
    mkdir -p "$OUT_DIR/trae"
    cp "$TRAE_TMP" "$OUT_DIR/trae/project_rules.md"
    info "Wrote integrations/trae/project_rules.md"
  fi
